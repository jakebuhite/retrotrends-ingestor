...
-- ---------------------------------------------------------------------------
-- NIGHTLY AGGREGATION: price_summaries
-- Run this as a scheduled job (e.g. AWS EventBridge + Lambda, or pg_cron).
-- Replace the period window as needed.
-- ---------------------------------------------------------------------------
-- Example: refresh monthly summaries for the trailing 12 months
/*
INSERT INTO price_summaries
    (game_id, variant, period_type, period_start, period_end,
     avg_sold_price, median_sold_price, min_sold_price, max_sold_price, sample_size)
SELECT
    game_id,
    variant,
    'month'                                         AS period_type,
    DATE_TRUNC('month', sold_at)::DATE              AS period_start,
    (DATE_TRUNC('month', sold_at) + INTERVAL '1 month - 1 day')::DATE AS period_end,
    ROUND(AVG(sold_price), 2)                       AS avg_sold_price,
    ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP
          (ORDER BY sold_price)::NUMERIC, 2)        AS median_sold_price,
...
WHERE status   = 'sold'
  ...
  AND sold_price BETWEEN 0.50 AND 5000  -- exclude obvious outliers
GROUP BY game_id, variant, DATE_TRUNC('month', sold_at)
ON CONFLICT (game_id, variant, period_type, period_start)
DO UPDATE SET
